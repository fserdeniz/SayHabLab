...
void mainCode(void)
{
	...
	// Start COMP1 comparator peripheral, we will use it in demodulation
	HAL_COMP_Start(&hcomp1);
	
	// Start COMP2 comparator peripheral, we will use it in demodulation
	HAL_COMP_Start(&hcomp2);
	
	// Start of the Lab Experiment
	// We send byte or bytes of arbitrary data
	// We use DAC_Channel_1 to output the BPSK modulation
	// We use DAC_Channel_2 to output the clock signal
	// We use COMP1 comparator to clear LPF output (demodulate BPSK)  
	// BPSK modulation pin PA4 (A2 on Nucleo-64)
	...
	for (uint16_t n = 0; n < N0; n++) 
	{
		...
		// For BPSK, "0" bit corresponds to 0 degree phase shift
		symbolValue = 0.49 * sinf(2 * PI * n / N0) + 0.5;
		// Convert the exact sample value to 16-bit integer value
		// DAC peripheral of the MCU accepts only integer numbers
	...
	{
		// Calculate the exact sample value of the symbol (floating number)
		// For BPSK, "1" bit corresponds to 180 degree phase shift
		symbolValue = -0.49 * sinf(2 * PI * n / N0) + 0.5;
		// Convert the exact sample value to 16-bit integer value
		// DAC peripheral of the MCU accepts only integer numbers
...
for (i = 0; i < 8; i++) 
{
    ...
    uint8_t bit = ((txData >> i) & 0x01);
    // Pay attention that we must transmit the symbol and clock simultaneously
    // Start digital to analog conversion on DAC Channel 1 (for symbol transmission)
    HAL_DAC_Start_DMA(&hdac, DAC_CHANNEL_1, (uint32_t*) &symbols[bit][0], N0, DAC_ALIGN_12B_R);
    // Start digital to analog conversion on DAC Channel 2 (for clock signal)
