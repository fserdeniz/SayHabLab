...
void mainCode(void)
{
	...
	for (uint16_t n = 0; n < N0; n++) 
	{
		...
		// Send Carrier signal for "0" bit (BFSK Modulation)
		...
		// By this, our Carrier signal will oscillate approximately between 0V and 3.3V (offset = 0.5*3.3 = 1.65V and 2*0.49*3.3 = 3.23 Vpp)
		// Carrier frequency (f0), Total sample count for each Symbol (N0), Discrete frequency of the Carrier (n0) and the sample rate of the DAC (fs)
		// f0 = fs / (N0/n0), below we have n0 = 5 -> Carrier frequency f0 = 500kHz/(250/5) = 500kHz/50 = 10kHz
		// symbolValue = 0.49 * sinf(2*PI*n0*n/N0) + 0.5
		symbolValue = 0.49*sinf(2*PI*5*n/N0) + 0.5;
