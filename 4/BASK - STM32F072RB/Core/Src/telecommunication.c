...
void mainCode(void)
{
	...
	// We use DAC_Channel_1 to output the BASK modulation
	// We use DAC_Channel_2 to output the clock signal
	// We use COMP1 comparator to clear LPF output (demodulate BASK)  
	// BASK modulation pin PA4 (A2 on Nucleo-64)
	...
	for (uint16_t n = 0; n < N0; n++) 
	{
		...
		// Logic low signal (voltage level = 0V) for "0" bit (or we can use any DC voltage instead)
		symbolValue = 0.0;
	...
	for (uint16_t n = 0; n < N0; n++) 
	{
		...
		// Send Carrier signal for "1" bit (BASK Modulation)
		...
		// Carrier frequency (fc), Total sample count for each Symbol (N0), Discrete frequency of the Carrier (nc) and the sample rate of the DAC (fs)
		// fc = fs / (N0/nc), below we have nc = 10 -> Carrier frequency fc = 500kHz/(250/10) = 500kHz/25 = 20kHz
		// symbolValue = 0.49 * sinf(2*PI*nc*n/N0) + 0.5
		symbolValue = 0.49*sinf(2*PI*10*n/N0) + 0.5;
		// Convert the exact sample value to 16-bit integer value
		// DAC perpheral of the MCU accepts only integer numbers
